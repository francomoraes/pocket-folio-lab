export default {
  dialog: {
    title: "Add Asset",
    titleEdit: "Edit Asset",
    fixedIncomeTitle: "Add Fixed Income Asset",
    fixedIncomeTitleEdit: "Edit Fixed Income Asset",
    operationTitle: "Log Operation",
    operationTitleEdit: "Edit Operation",
  },
  fields: {
    operation: "Operation",
    ticker: "Ticker",
    quantity: "Quantity",
    price: "Price",
    date: "Date",
    institution: "Institution",
    notes: "Notes",
    assetType: "Asset Type",
    averagePrice: "Average Price",
    currency: "Currency",
    description: "Description",
    startDate: "Start Date",
    maturityDate: "Maturity Date",
    indexationMode: "Indexation Type",
    interestRate: "Rate (% p.a.)",
    additionalRate: "Additional Rate (%)",
    investedValue: "Invested Value",
    currentPrice: "Current Price (manual)",
    assetTypeName: "Asset Type",
    unitPrice: "Unit Price",
    fees: "Fees (optional)",
    totalAmount: "Amount Received",
    asset: "Asset",
    newAsset: "+ Register new asset",
  },
  operations: {
    buy: "Buy",
    sell: "Sell",
    dividend: "Dividend",
  },
  history: {
    title: "Operation History",
    addOperation: "Log Operation",
    filters: {
      asset: "Asset",
      allAssets: "All assets",
      type: "Type",
      allTypes: "All",
      dateFrom: "From",
      dateTo: "To",
      clear: "Clear filter",
      clearAll: "Clear filters",
    },
    columns: {
      date: "Date",
      asset: "Asset",
      type: "Type",
      quantity: "Qty",
      unitPrice: "Price",
      fees: "Fees",
      total: "Total",
    },
    empty: "No operations logged yet.",
    emptyFiltered: "No operations match this filter.",
  },
  placeholders: {
    selectAsset: "Select an asset",
    ticker: "AAPL",
    quantity: "100",
    price: "0.00",
    selectInstitution: "Select an institution",
    selectType: "Select type",
    description: "Ex: XYZ Bank CDB",
    interestRate: "Ex: 12.5",
    investedValue: "Ex: 10000.00",
    name: "Ex: Stocks",
    targetPercentage: "Ex: 50",
  },
  currency: {
    brl: "BRL (Real)",
    usd: "USD (Dollar)",
  },
  indexationMode: {
    pre: "Fixed Rate",
    cdi: "CDI +",
    ipca: "IPCA +",
    selic: "Selic +",
  },
  emptyState: {
    noTypes: "No asset types registered.",
    noTypesLine2: "Go to Settings to add them.",
    noInstitutions: "No institutions registered.",
    noInstitutionsLine2: "Go to Settings to add them.",
    settingsLink: "Go to Settings →",
  },
  fixedIncome: {
    manualMode: "I don't know the rate/dates (manual update)",
    currentValue: "Current Value (manual)",
    currentValueHint:
      "Fill in the current value to record it manually. If you enter a rate, this field is calculated automatically.",
    interestRateDisabledTooltip:
      'Clear the "Current Value" field to enable the rate.',
    currentValueDisabledTooltip:
      "Clear the interest rate to enable the manual current value.",
  },
  messages: {
    success: "Asset added successfully",
    error: "Error processing asset. Please try again.",
    created: "Asset created successfully!",
    updated: "Asset updated successfully!",
    deleted: "Asset deleted successfully!",
    addError: "Error adding asset.",
    updateError: "Error updating asset.",
    deleteError: "Error deleting asset.",
    retryPrice: "Fetch Price",
    retryPriceSuccess: "Price updated successfully!",
    retryPriceError: "Could not fetch the price. Try again later.",
    refreshPricesSuccess: "Market prices updated successfully!",
    refreshPricesError: "Error updating market prices.",
    operationCreated: "Operation logged successfully!",
    operationUpdated: "Operation updated successfully!",
    operationDeleted: "Operation deleted successfully!",
    operationCreateError: "Error logging operation.",
    operationUpdateError: "Error updating operation.",
    operationDeleteError: "Error deleting operation.",
    futureDateNotAllowed: "You cannot log an operation with a future date.",
  },
};
